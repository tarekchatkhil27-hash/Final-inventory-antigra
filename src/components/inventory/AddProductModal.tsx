...
import { Product, Supplier, HistoryLog, Payable, PurchaseRecord, Transaction } from '../../types';
...
export function AddProductModal({ isOpen, onClose }: AddProductModalProps) {
  ...
  const handleCloseAll = () => {
    ...
    setProductsToAdd([]);
    setFormData({ name: '', sku: '', brand: '', category: '', size: '', color: '', design: '', price: '', cost: '', quantity: '', minThreshold: '', discount: '', discountType: 'percent' });
          ...
          <section className="space-y-4 bg-slate-50 dark:bg-slate-900/50 p-4 rounded-xl border border-slate-200 dark:border-slate-800">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-1.5">
                ...
                <input name="name" value={formData.name} onChange={handleInputChange} className="w-full px-3 py-1.5 text-sm focus:ring-2 dark: dark: border-none bg-slate-100 dark:bg-slate-800 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500/30 transition-all duration-200 focus:outline-none dark:text-slate-50" />
              </div>
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-slate-700 dark:text-slate-300">{t('SKU')} *</label>
                <input name="sku" value={formData.sku} onChange={handleInputChange} className="w-full px-3 py-1.5 text-sm focus:ring-2 dark: dark: border-none bg-slate-100 dark:bg-slate-800 rounded-xl focus:bg-white focus:ring-2 focus:ring-indigo-500/30 transition-all duration-200 focus:outline-none dark:text-slate-50" />
              </div>
              <div className="space-y-1.5">
